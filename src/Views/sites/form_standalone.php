...
<div class="max-w-xl space-y-6">
    ...
    <h1 class="text-xl font-semibold text-slate-800"><?= $isEdit ? 'Edit Site' : 'Add Site' ?></h1>
    ...
    <form method="POST"
        ...
        <div>
            ...
            <select id="client_id" name="client_id" onchange="filterDomains()"
                ...
                <?php foreach ($clients as $c): ?>
                    <option value="<?= $c['id'] ?>" <?= ($site['client_id'] ?? null) == $c['id'] ? 'selected' : '' ?>>
                        <?= e($c['name']) ?>
                    </option>
                <?php endforeach ?>
            </select>
            ...
            <div>
                ...
                <select id="domain_id" name="domain_id"
                    ...
                    <?php foreach ($domains as $d): ?>
                        <option value="<?= $d['id'] ?>"
                                data-client="<?= $d['client_id'] ?>"
                                <?= ($site['domain_id'] ?? null) == $d['id'] ? 'selected' : '' ?>>
                            <?= e($d['domain']) ?>
                        </option>
                    <?php endforeach ?>
                </select>
            ...
            <div>
                ...
                <select id="server_id" name="server_id"
                    ...
                    <?php foreach ($servers as $s): ?>
                        <option value="<?= $s['id'] ?>" <?= ($site['server_id'] ?? null) == $s['id'] ? 'selected' : '' ?>>
                            <?= e($s['name']) ?>
                        </option>
                    <?php endforeach ?>
                </select>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" id="website_stack" name="website_stack" value="<?= e($site['website_stack'] ?? '') ?>"
                       placeholder="e.g. WordPress, Craft CMS"
                       class="w-full border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500">
            </div>
            <div>
                ...
                <input type="text" id="css_framework" name="css_framework" value="<?= e($site['css_framework'] ?? '') ?>"
                       placeholder="e.g. Tailwind, Bootstrap"
                       class="w-full border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500">
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" id="smtp_service" name="smtp_service" value="<?= e($site['smtp_service'] ?? '') ?>"
                   placeholder="e.g. Mailgun, Brevo, Postmark"
                   class="w-full border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500">
        </div>
        ...
        <div>
            ...
            <select id="ploi_site_id" name="ploi_site_id"
                    class="w-full border border-slate-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500">
                <option value="">— None —</option>
                <?php foreach (($ploiSites ?? []) as $ps): ?>
                    <option value="<?= $ps['id'] ?>"
                            <?= (($currentPloiId ?? null) == $ps['id']) ? 'selected' : '' ?>>
                        <?= e($ps['domain']) ?><?= $ps['is_stale'] ? ' (stale)' : '' ?>
                    </option>
                <?php endforeach ?>
            </select>
        </div>
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <a href="/sites<?= $isEdit ? '/' . $site['id'] : '' ?>" class="text-sm text-slate-500 hover:text-slate-800">Cancel</a>
        </div>
...
<script>
// Filter domain dropdown to only show domains for the selected client
...
// Run on page load if editing with a pre-selected client
