...
class DashboardController
{
    ...
    public function index(): void
    {
        ...
        // ── Needs attention (high + medium); housekeeping count links to /today
        $attention         = new \CoyshCRM\Services\Attention($this->db);
        $allAttention      = $attention->items(true);
        $attentionItems    = array_values(array_filter($allAttention, fn($i) => $i['severity'] !== 'low'));
        $attentionLowCount = count($allAttention) - count($attentionItems);

        $includeCharts = true;

        render('dashboard.index', compact(
            'includeCharts', 'attentionItems', 'attentionLowCount',
